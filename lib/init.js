...
function _initStatics() {
    if (_.isEqual(config.setters['stylesheet engine'], 'stylus')){
...
function _initLoginVerifier() {
    if(config.site.private){
        app.use(function(req, res, next){
            if(req.session.loggedIn || _.contains(config.site.publicPaths, req.path)){
                next();
            }
            else{
...
function _initHttps(){
    if(config.uses.https){
        var privateKey  = fs.readFileSync(config.uses.certPath + 'server.key', 'utf8'),
            certificate = fs.readFileSync(config.uses.certPath + 'server.crt', 'utf8'),
