...
function _initApp() {
    ...
    app.tearDown = _tearDown;        // Or perhaps the user screwed up the settings/routes/db?
...
function _initMiddleware() {
    ...
    app.use(logger);
    //DEPRECATED   app.use(express.bodyParser({KeepExtensions: true, uploadDir: config.uses.tmpDir}));
